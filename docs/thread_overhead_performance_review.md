...
> **0.10.3 勘误（2026-07-15）**：本文只分析 AVIF grid/tile 路径，包含尚未用当前固定输入实测的加速估算，不能用于解释普通 AOM 批处理回归。0.10.3 canonical CLI 使用相同输入指纹、AOM 3.13.3、q80、speed6、420、8-bit、12 线程预算、平衡电源和 Release 构建；613 张五轮墙钟中位/P95 为 146.235/215.123 s，进程 CPU 中位/P95 为 1332.266/1392.453 s，encode 阶段仍约 97.0%。对比 0.10.2 严格回归没有复现，而且 0.10.3 未修改 CLI 编码路径，因此未执行 ETW/perf、未调整 AOM 参数，也不把更快中位数归因于 UI 改动。本文 Phase 估算继续视为历史假设，实施前必须重新 benchmark。
