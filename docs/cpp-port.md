...
- 移除了 ffmpeg/ffprobe 后端；当前质量搜索以 native 视觉质量评分为主，不再保留旧的自动搜索入口。
...
- `avif.core`: UTF-8/宽字符转换、图片扫描、日志、CSV 和少量 Win32 工具；Win32 分配的错误消息缓冲区由智能指针释放。
- `avif.magick_backend`: MagickWand 运行时解析、环境变量配置、AVIF/WebP/JXL 编码；MagickWand、Magick 字符串列表和 libheif handle 使用 RAII 托管。
...
- C API、Win32 和 COM 交界处统一用 `std::unique_ptr` 自定义 deleter 或局部 RAII 类型表达拥有关系，覆盖 `Release`、`CoTaskMemFree`、`LocalFree`、`MagickRelinquishMemory` 和 libheif release 路径。
- 只在所有权或跨线程时序不直观的地方保留中文注释，避免普通业务流程被注释噪音淹没。
