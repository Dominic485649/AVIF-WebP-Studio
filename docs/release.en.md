...
4. Build, archive, and publicly upload `AWJ_Win.7z` and `AWJ_Linux.7z` from a clean 1.0.6 tag. After assets are public, run `scripts/package-release.ps1` with explicit `-ManifestKeyId`, `-ManifestExpiresAtUtc`, matching release public key/seed, and the preceding manifest public key when it differs. The script rejects a signer outside the non-revoked keyring entry.
