...
- Added a clickable version label, persistent update dot, and a first-class changelog page beside Queue, Parameters, and Settings. Settings now expose the update channel, changelog visibility, a manual check, the last successful check time, and status. Hovering, opening the page, navigating, restarting, or a failed check never clears the dot.
