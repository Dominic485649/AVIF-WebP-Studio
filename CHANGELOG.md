...
- 新增可重复 Windows Release benchmark：固定输入指纹、AOM/q80/speed6/420/8-bit、电源方案和构建/commit，每组预热 1 次、测量 5 次，报告墙钟/CPU/P95/峰值内存/吞吐量/版本/依赖版本及 decode/prepare/encode/write 阶段耗时。
- 0.10.3 按 CLI-only 约定完成 1/4/12/13/613 张、透明与不透明矩阵；613 张中位墙钟 146.235 s、P95 215.123 s、进程 CPU 中位 1332.266 s、吞吐 4.192 img/s、encode 占 97.0%。轮间波动仍大，且编码路径未改，因此不把结果归因于 UI 改动，不触发 ETW/perf，也未实施无证据的 AOM 微优化。
