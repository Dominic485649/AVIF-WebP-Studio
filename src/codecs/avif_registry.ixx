...
AppliedBitDepth applied_bit_depth_for(const AvifEncoderCapability& capability,
  ...
  if (requested) {
    return AppliedBitDepth{.value = *requested,
                           .reason = "explicit bit-depth requested"};
  }
...
bool capability_matches(const AvifEncoderCapability& capability,
  ...
  const auto bit_depth = applied_bit_depth_for(capability, request.requested_bit_depth);
...
std::string explicit_rejection_reason(const AvifEncoderCapability& capability,
  ...
  if (!contains_chroma(capability, chroma)) {
    if (capability.mode == AvifEncoderMode::svt) {
      return "SVT AVIF encoder only supports 420 chroma; use --chroma 420/auto or --avif-encoder aom.";
    }
  ...
  if (capability.mode == AvifEncoderMode::svt &&
      request.pixel_count > encoding_defaults::svt_safe_max_pixels) {
    return std::format("SVT AVIF encoder is disabled for images over {} pixels; use --avif-encoder auto/aom.",
...
export std::vector<AvifEncoderCapability> avif_encoder_capabilities_for_experimental(
  ...
  return {
      AvifEncoderCapability{.mode = AvifEncoderMode::svt,
                            ...
                            .bit_depths = {8, 10},
                            .enabled = false,
                            .unavailable_reason = "AVIF encoder svt is not available in this libavif build; the current vcpkg libavif port does not provide an SVT feature.",
                            .license = "BSD-3-Clause",
                            .default_speed = encoding_defaults::default_svt_preset},
...
export std::expected<AvifEncoderSelection, std::string> select_avif_encoder_from_capabilities(
  ...
  for (const auto mode : {AvifEncoderMode::svt, AvifEncoderMode::aom}) {
    const auto it = std::ranges::find(capabilities, mode, &AvifEncoderCapability::mode);
    if (it == capabilities.end() || !avif_registry_detail::capability_enabled_for_auto(*it) ||
        !avif_registry_detail::capability_matches(*it, request)) {
      continue;
    }
    std::string reason;
    if (it->mode != AvifEncoderMode::svt) {
      reason = "auto excluded SVT for the requested chroma, bit-depth, pixel count, or build availability.";
    }
    return avif_registry_detail::make_selection(*it, request, std::move(reason));
  }

  return std::unexpected{"No AVIF encoder supports the requested parameters."};
}
...
SpeedMapping avif_speed_mapping_for(const AvifEncoderSelection& selection) {
  switch (selection.applied_encoder) {
    ...
    case AvifEncoderMode::automatic:
    default:
      return map_avif_speed_to_svt_preset(selection.speed);
  }
}
