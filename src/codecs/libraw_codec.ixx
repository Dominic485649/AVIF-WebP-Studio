...
int open_file(LibRaw& raw, const fs::path& path) {
#ifdef _WIN32
  return raw.open_file(path.native().c_str());
#else
  return raw.open_file(path.string().c_str());
#endif
}
...
std::expected<void, std::string> reject_multiple_raw_images(
    const LibRaw& raw,
    std::string_view source_name) {
  if (raw.imgdata.idata.raw_count > 1) {
    return std::unexpected{std::format("暂不支持多图 RAW 输入: {}", source_name)};
  }
  return {};
}
...
export class LibRawImageDecoder final : public ImageDecoder {
 public:
  ...
  std::expected<ImageDimensions, std::string> probe_dimensions(
    ...
    try {
      if (auto file_size = libraw_detail::check_input_file_size(path); !file_size) {
        return std::unexpected{file_size.error()};
      }
      LibRaw raw;
      const auto opened = libraw_detail::open_file(raw, path);
      if (opened != LIBRAW_SUCCESS) {
        return std::unexpected{std::format("LibRaw 打开失败: {}", libraw_strerror(opened))};
      }
      if (auto images = libraw_detail::reject_multiple_raw_images(raw, display_path_for_user(path));
          !images) {
      ...
      return decoder_common::make_image_dimensions_checked(static_cast<std::uint64_t>(width),
                                                           static_cast<std::uint64_t>(height),
                                                           "LibRaw");
    } catch (const std::bad_alloc&) {
      return std::unexpected{"LibRaw 尺寸探测内存不足。"};
    } catch (const std::length_error&) {
      return std::unexpected{"LibRaw 尺寸探测数据超过运行时限制。"};
    } catch (const std::filesystem::filesystem_error&) {
      return std::unexpected{"LibRaw 尺寸探测文件系统访问失败。"};
    }
  }
  ...
  std::expected<ImageDecodeResult, std::string> decode(const fs::path& path) const override {
    try {
      ...
      auto rgba = decoder_common::make_byte_buffer(*byte_count, "LibRaw decoder");
      if (!rgba) {
      ...
      return std::unexpected{"LibRaw 解码内存不足。"};
    } catch (const std::length_error&) {
      return std::unexpected{"LibRaw 解码数据超过运行时限制。"};
    } catch (const std::filesystem::filesystem_error&) {
      return std::unexpected{"LibRaw 解码文件系统访问失败。"};
    }
  }
};
