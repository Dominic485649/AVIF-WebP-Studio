...
这是原 C# AVIF/WebP 压缩控制台的 C++23 迁移版。项目现在以 CMake 为主构建系统，核心转换逻辑同时服务于两个入口：
...
- `-d, --bit-depth <8|10|12>`: AVIF 可选输出位深，留空保持原片；WebP 固定为 `8`
- `--chroma auto|444|422|420`: AVIF 色度采样，默认 `auto`；也可用 `--444`、`--422`、`--420`
...
- `--speed <0-10>`: 可选，映射到 `MagickSetOption("heic:speed", value)`
...
当前后端遵循 ImageMagick/libheif 与 WebP coder 的能力边界：
...
- 如果原图是 4:4:4、4:2:2、4:2:0：AVIF 在 `auto` 能识别源采样时会分别写入 444、422、420，手动选择时按 `--chroma` 写入；WebP 有损会变成 4:2:0，WebP 无损保持 ARGB 语义。
